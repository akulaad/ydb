# Functions of built-in C++ libraries

Many application functions that on the one hand are too specific to become part of the YQL core, and on the other hand might be useful to a wide range of users, are available through built-in C++ libraries.

In addition to built-in libraries, {{ ydb-short-name }} supports [user-defined UDFs](../../../dev/udf-store/index.md) uploaded via UDF Store (including [WASM UDFs](../../../dev/udf-store/wasm/index.md)).

* [DateTime](datetime.md)
* [Digest](digest.md)
* [Histogram](histogram.md)
* [Hyperscan](hyperscan.md)
* [Ip](ip.md)
* [Knn](knn.md)
* [Math](math.md)
* [Pcre](pcre.md)
* [Pire](pire.md)
* [Re2](re2.md)
* [Roaring](roaring.md)
* [String](string.md)
* [Unicode](unicode.md)
* [Url](url.md)
* [Yson](yson.md)

