# Функции встроенных C++ библиотек

Многие прикладные функции, которые с одной стороны слишком специфичны, чтобы стать частью ядра YQL, а с другой — могут быть полезны широкому кругу пользователей, доступны через встроенные C++ библиотеки.

Помимо встроенных библиотек, {{ ydb-short-name }} поддерживает [пользовательские UDF](../../../../dev/udf-store/index.md), загружаемые через UDF Store (в том числе [WASM UDF](../../../../dev/udf-store/wasm/index.md)).

* [DateTime](datetime.md)
* [Digest](digest.md)
* [Histogram](histogram.md)
* [Hyperscan](hyperscan.md)
* [Ip](ip.md)
* [Knn](knn.md)
* [Math](math.md)
* [Pcre](pcre.md)
* [Pire](pire.md)
* [Re2](re2.md)
* [Roaring](roaring.md)
* [String](string.md)
* [Unicode](unicode.md)
* [Url](url.md)
* [Yson](yson.md)
